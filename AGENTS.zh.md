...
- `skills/obsidian-cli.md`
  ...
  触发短语包括："scan the vault"、"批量检查这些笔记"、"find notes missing type"、"run Obsidian CLI"、"统计一下这个 vault"
...
- `type/status/area/project/source` 用于**结构化分类与过滤**（Bases 与搜索）。
- `tags` 用于**关键词 / 主题**（例如 `openai`、`agent`、`coding`），服务于搜索与联想。避免把结构化分类也编码进 tags，减少冗余。
- 字符串包含特殊字符时要加引号，确保 YAML 合法。

字段约定（v1）：

- `type`：作为 Bases 里的主分类字段；类型集合允许后续演进，但不要频繁修改同一笔记的 `type`。
...
- 对于同一 `type` 的笔记，优先使用一致的模板（位于 `templates/`），以减少缺项并提升可读性。

### 模板（v1）

- 所有模板放在 `templates/` 中。
- 按类型结构组织模板。每种类型至少应有一个（或多个）模板；创建该类型笔记时优先从模板开始。
...
- 更详细的链接启发式、红链策略与 hub 规则，放在 `obsidian-markdown` 中。
...
- **inbox / 待整理**：缺少 `type` 或关键属性、且近期新建但尚未归类的笔记。
...
- 每周：清理 Inbox（补 `type/tags`、补 1–3 个关键链接）；用 Missing 视图挑选需要处理的笔记，避免 Missing 持续膨胀。
...
2) **提出 taxonomy 草案**：给出核心 properties（例如 `type/status/area/project/source`）与关键词 tags 的建议；只有在用户明确要求时才补 namespace tags。
